...
const PreSeedDetail = () => {
  ...
  const [preseed, setPreSeed] = React.useState<PreSeedTypes | null>(null);
  ...
  const fetchCompanyDetails = async (preseedId: string) => {
    try {
      const response = await PreSeedService.getPreSeedInvestorsById(preseedId);
      if (response.success && response.data) {
        setPreSeed(response.data);
      } else {
        notification.error({
          message: "Error",
          ...
          <Col xs={24} lg={16}>
            <div className="space-y-6">
              ...
              <div>
                ...
                {preseed.preferredStartupStage.map((stage) => (
                  <Tag
                    key={stage}
                    color="green"
              ...
              <div>
                ...
                {preseed.investedCompanies &&
                preseed.investedCompanies.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {preseed.investedCompanies.map((company, index) => (
                      <div
                        key={index}
            ...
            <div className="rounded-xl overflow-hidden bg-white shadow-lg border border-gray-100">
              ...
              <div className="p-6 space-y-4 bg-gray-50">
                ...
                <div className="space-y-3">
                  {/* LinkedIn Button */}
                  {preseed.socialLinks.linkedin && (
                    <button
                      onClick={() =>
                        window.open(preseed.socialLinks.linkedin, "_blank")
                      }
                  ...
                  <div className="flex gap-4 pt-2">
                    ...
                    {preseed.socialLinks.twitter && (
                      <a
                        href={preseed.socialLinks.twitter}
                        target="_blank"
                        rel="noopener noreferrer"
