...
const PreSeed = () => {
  ...
  const [modalContent, setModalContent] = useState<PreSeedTypes | null>(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [loading, setLoading] = useState(false);
  const [preseeds, setPreseeds] = useState<PreSeedTypes[]>([]);
  ...
  const handleCompanyClick = (id: string) => {
    navigate(`/portal/preseed-investors/preseed-detail/${id}`);
  };
  ...
  useEffect(() => {
    if (preseeds.length > 0) {
      const sectors = _.uniq(preseeds.flatMap((c) => c.sector));
      const locations = _.uniq(preseeds.map((c) => c.preseedLocation));
      const industries = _.uniq(preseeds.flatMap((c) => c.focusIndustries));
      const stages = _.uniq(preseeds.flatMap((c) => c.preferredStartupStage));
  ...
  const applyFilters = (preseeds: PreSeedTypes[]) => {
    return preseeds.filter((preseeds: PreSeedTypes) => {
      // First apply search query
      const searchLower = searchQuery.toLowerCase();
      const matchesSearch =
        !searchQuery || // if no search query, return true
        preseeds.preseedName.toLowerCase().includes(searchLower) ||
        preseeds.sector.some((s) => s.toLowerCase().includes(searchLower)) ||
        preseeds.focusIndustries.some((industry) =>
      ...
      if (
        ...
        !preseeds.sector.some((s) => filters.sectors.includes(s))
      ) {
        return false;
      }
      ...
      if (
        ...
        !filters.locations.includes(preseeds.preseedLocation)
      ) {
        return false;
      }
      ...
      if (
        ...
        !preseeds.focusIndustries.some((i) => filters.industries.includes(i))
      ) {
        return false;
      }
      ...
      if (
        ...
        !filters.startupStage.some((stage) =>
          preseeds.preferredStartupStage.includes(stage)
        )
      ) {
        return false;
      }
  ...
  const fetchCompanies = async () => {
    setLoading(true);
    try {
      const response = await PreSeedService.getPublicPreSeedInvestors();
      if (response.success) {
        console.log("Fetched data:", response.data);
        setPreseeds(response.data || []);
      } else {
        notification.error({
          message: "Error",
          description: response.message || "Failed to fetch preseeds",
        });
      }
    } catch (error) {
      notification.error({
  ...
  const showModal = (preseed: PreSeedTypes) => {
    console.log("Opening modal for preseeds Investor:", preseed);
    setModalContent(preseed);
  ...
  const columns = [
    {
      ...
      render: (_: any, record: PreSeedTypes) => (
        <div className="flex items-center space-x-3">
          <img
            src={record.preseedLogo}
            alt={record.preseedName}
          ...
          >
            <div className="font-medium">{record.preseedName}</div>
            <div className="text-sm text-gray-500">
              {record.preseedLocation}
            </div>
          </div>
        </div>
      ),
  ...
  return (
    <div className="max-w-8xl mx-auto ">
      <div className="flex justify-between items-center mb-6">
        <Title level={2}>Pre-Seed Investors</Title>
        <Switch
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-4 gap-4 md:gap-6 lg:gap-8">
          {applyFilters(preseeds).map((preseed) => (
            <div key={preseed._id}>
              <Card
                hoverable
              ...
              >
                <div className="flex flex-col h-full relative">
                  ...
                  <Button
                    type="default"
                    icon={<Info className="h-4 w-4" />}
                    onClick={() => showModal(preseed)}
                    className="absolute top-0 right-0"
                  ></Button>

                  {/* Visit Button */}
                  <a
                    href={preseed.socialLinks.website}
                    target="_blank"
                  ...
                  <div className="block space-y-4 md:space-y-0 items-start gap-4 mb-4">
                    <div
                      ...
                      onClick={() => handleCompanyClick(preseed._id)}
                    >
                      <img
                        src={preseed.preseedLogo}
                        alt={preseed.preseedName}
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {preseed.focusIndustries.map((industry) => (
                        <Tag
                          key={industry}
                          color="cyan"
                          className="rounded-sm m-1"
        ...
        <Table
          columns={columns}
          dataSource={applyFilters(preseeds)}
          rowKey="_id"
          pagination={{
            pageSize: 10,
            responsive: true,
        ...
        {modalContent && (
          <div className="p-6 dark">
            <div className="flex items-start space-x-6 mb-8">
              <img
                src={modalContent.preseedLogo}
                alt={modalContent.preseedName}
              ...
              <div>
                ...
                >
                  {modalContent.preseedName}
                </Title>
                <div className="flex items-center space-x-2 mb-4">
                  <div className="flex items-center space-x-2">
                    <span style={{ color: "rgba(255, 255, 255, 0.45)" }}>
                      {modalContent.preseedLocation}
                    </span>
                  </div>
                </div>
