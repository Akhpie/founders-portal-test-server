...
import PreSeedService from "../../../services/preseedService";
...
const PreSeedManagement: React.FC = () => {
  const [companies, setCompanies] = useState<PreSeedTypes[]>([]);

  const [loading, setLoading] = useState(false);
  const [modalVisible, setModalVisible] = useState(false);
  const [editingCompany, setEditingCompany] = useState<PreSeedTypes | null>(
    null
  );
  const [form] = Form.useForm();
  const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  const [filteredCompanies, setFilteredCompanies] = useState<PreSeedTypes[]>(
    []
  );
  ...
  const handleSearch = (value: string) => {
    ...
    const filtered = companies.filter((preseed) => {
      return (
        preseed.preseedName?.toLowerCase().includes(searchLower) ||
        preseed.preseedLocation?.toLowerCase().includes(searchLower) ||
        preseed.sector?.some((s) => s.toLowerCase().includes(searchLower)) ||
        preseed.preseedType?.toLowerCase().includes(searchLower) ||
        preseed.focusIndustries?.some((i) =>
          i.toLowerCase().includes(searchLower)
        ) ||
        preseed.about?.toLowerCase().includes(searchLower)
      );
    });
  ...
  const handleImport = async (dataArray: any[]) => {
    try {
      ...
      for (const data of dataArray) {
        const response = await PreSeedService.createPreSeedInvestors(data);
        if (!response.success) {
          notification.error({
  ...
  const fetchCompanies = async () => {
    setLoading(true);
    try {
      const response = await PreSeedService.getAdminPreSeedInvestors();
      if (response.success) {
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      const payload = {
        preseedLogo: values.preseedLogo,
        preseedName: values.preseedName,
        preseedLocation: values.preseedLocation,
        preseedType: values.preseedType,
      ...
      if (editingCompany) {
        const response = await PreSeedService.updatePreSeedInvestors(
          editingCompany._id,
          payload
        );
        ...
        if (response.success) {
          notification.success({
            message: "Success",
            description: "PreSeed investor updated successfully",
          });
          ...
          notification.error({
            message: "Error",
            description:
              response.message || "Failed to update PreSeed investor",
          });
        }
      } else {
        const response = await PreSeedService.createPreSeedInvestors(payload);

        if (response.success) {
          notification.success({
            message: "Success",
            description: "PreSeed investor created successfully",
          });
          setModalVisible(false);
          form.resetFields();
          fetchCompanies();
          ...
          notification.error({
            message: "Error",
            description:
              response.message || "Failed to create PreSeed investor",
          });
        }
      }
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    try {
      const response = await PreSeedService.deletePreSeedInvestors(id);
      if (response.success) {
        notification.success({
          message: "Success",
  ...
  const columns = [
    ...
    {
      title: "Preseed Name",
      dataIndex: "preseedName",
      key: "preseedName",
    },
    {
      title: "Location",
      dataIndex: "preseedLocation",
      key: "preseedLocation",
    },
    {
      title: "Sector",
    ...
    },
    {
      title: "Actions",
      key: "actions",
      render: (_: any, record: PreSeedTypes) => (
        <Space>
          <Button
            icon={<EditOutlined />}
            onClick={() => {
  ...
  const handleBulkDelete = async () => {
    try {
      const deletePromises = selectedRowKeys.map((key) =>
        PreSeedService.deletePreSeedInvestors(key.toString())
      );
  ...
  const prepareExportData = (selectedOnly: boolean = false) => {
    const dataToExport = selectedOnly
      ? companies.filter((preseed) => selectedRowKeys.includes(preseed._id))
      : companies;

    return dataToExport.map((preseed) => ({
      "PreSeed Logo": preseed.preseedLogo,
      "PreSeed Name": preseed.preseedName,
      Location: preseed.preseedLocation,
      Sector: Array.isArray(preseed.sector)
        ? preseed.sector.join(", ")
        : preseed.sector,
      "PreSeed Type": preseed.preseedType,
      "Focus Industries": preseed.focusIndustries.join(", "),
      NoOfInvestedCompanies: preseed.noOfInvestedCompanies,
      About: preseed.about,
      Website: preseed.socialLinks?.website,
      LinkedIn: preseed.socialLinks?.linkedin,
      Twitter: preseed.socialLinks?.twitter,
      Active: preseed.isActive ? "Yes" : "No",
      "Invested Companies": preseed.investedCompanies
        ? preseed.investedCompanies.map((c) => c.companyName).join(", ")
        : "",
    }));
  };
  ...
  const exportToCSV = (selectedOnly: boolean = false) => {
    ...
    link.setAttribute(
      "download",
      `preseed_investors${selectedOnly ? "_selected" : ""}.csv`
    );
  ...
  const exportToExcel = (selectedOnly: boolean = false) => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "PreSeed Investors");
    XLSX.writeFile(
      wb,
      `preseed_investors${selectedOnly ? "_selected" : ""}.xlsx`
    );
  };
  ...
  const ImportSection: React.FC<{ onDataImported: (data: any) => void }> = ({
    ...
    const transformImportedData = (data: any) => {
      return {
        preseedLogo: data["PreSeed Logo"],
        preseedName: data["PreSeed Name"],
        preseedLocation: data["Location"],
        sector: data["Sector"]?.split(",").map((s: string) => s.trim()),
        preseedType: data["PreSeed Type"],
      ...
      >
        ...
        >
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item
                ...
                label="PreSeed Logo"
                rules={[
                  { required: true, message: "Please upload PreSeed logo" },
                ]}
              >
            ...
            <Col span={12}>
              <Form.Item
                ...
                label="PreSeed Name"
                rules={[
                  { required: true, message: "Please enter PreSeed name" },
                ]}
              >
                <Input />
              </Form.Item>
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item
                name="preseedLocation"
                label="PreSeed Location"
                rules={[
                  { required: true, message: "Please enter PreSeed name" },
                ]}
              >
            ...
            <Col span={12}>
              ...
              >
                <Select>
                  <Option value="Pre-Seed">PreSeed Investor</Option>
                </Select>
              </Form.Item>
            </Col>
            <Col span={12}>
          ...
          <Form.Item
            ...
            label="About"
            rules={[
              { required: true, message: "Please enter preseed description" },
            ]}
          >
            <TextArea rows={4} />
          </Form.Item>
...
export default PreSeedManagement;
